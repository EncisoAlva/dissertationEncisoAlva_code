...
for condition = 1:nConditions
  ...
  for solverIDX = 1:nSolvers
    ...
    % 3 extra data: solvername, SNR, depth of true barycenter
    data = zeros(nCases,nEvals +2 +3);
    data(:, 2+(1:(nEvals+3)) ) = evaluation.(currSolver);
    TABLEtmp = array2table(data, ...
      'VariableNames', cat(2,{'Solver', 'SNR','Depth'},evaluation.ExtraNames,EvalNames') );
